...
- The main menu uses explicit desktop, tablet, phone-portrait, phone-landscape, and ultra-compact profiles. Its title lockup, mission settings, tuning controls, run selection, and status are ordered bands; position later bands from those bounds rather than guessing offsets from text anchors.
