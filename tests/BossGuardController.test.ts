...
describe('BossGuardController', () => {
  test('fills to capacity and requests one break', () => {
    expect(applyBossGuardHit({ ...idle, value: 2 }, 2, 100)).toMatchObject({
      value: 4,
      lastHitAt: 100,
      shouldBreak: true,
    });
  ...
  test('freezes an active break then resets on recovery', () => {
    ...
    expect(advanceBossGuard(broken, { time: 2499, delta: 16, decayDelayMs: 1500, decayPerSecond: 10 })).toMatchObject({
      frozen: true,
      recovered: false,
    });
    expect(advanceBossGuard(broken, { time: 2500, delta: 16, decayDelayMs: 1500, decayPerSecond: 10 })).toMatchObject({
      value: 0,
      broken: false,
      frozen: false,
      recovered: true,
